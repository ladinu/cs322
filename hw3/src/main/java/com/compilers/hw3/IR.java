...
class IR {
  ...
  static Map<Reg,X86.Reg> getPreferences(Func func) {
    Map<Reg,X86.Reg> preferences = new HashMap<Reg,X86.Reg>();

    // Incoming arguments from callee's perspective
    for (int i = 0; i < func.params.length; i++)
    preferences.put(new Id(func.params[i]),X86.argRegs[i]);

    for (Inst c : func.code) {
      if (c instanceof Call) {
        Call cl = (Call) c;
        // Arguments from caller's perspective
        for (int i = 0; i < cl.args.length; i++) {
          Src argRand = cl.args[i];
          if (argRand instanceof Reg)
            preferences.put((Reg) argRand,X86.argRegs[i]);
        }
        // Return value from caller's perspective
        if (cl.rdst instanceof Reg)
          preferences.put((Reg) cl.rdst,X86.RAX);
      } else if (c instanceof Return) {
        // Return value from callee's perspective
        Return r = (Return) c;
        if (r.val instanceof Reg)
          preferences.put((Reg) r.val,X86.RAX);
      } else if (c instanceof Binop) {
        Binop b = (Binop) c;
        if (b.op == ArithOP.DIV) {
          // Argument and result of DIV
          if (b.src1 instanceof Reg)
            preferences.put((Reg) b.src1,X86.RAX);
          if (b.dst instanceof Reg)
            preferences.put((Reg) b.dst,X86.RAX);
        }
      }
    }
    return preferences;
